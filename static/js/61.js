...
/***/ "./node_modules/_maptalks.markercluster@0.8.3@maptalks.markercluster/dist/maptalks.markercluster.es.js":
/*!*************************************************************************************************************!*\
  !*** ./node_modules/_maptalks.markercluster@0.8.3@maptalks.markercluster/dist/maptalks.markercluster.es.js ***!
  \*************************************************************************************************************/
/*! exports provided: ClusterLayer */
...
/* harmony import */ var maptalks__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! maptalks */ "./node_modules/_maptalks@0.47.0@maptalks/dist/maptalks.es.js");
/*!
...
var ClusterLayer = function (_maptalks$VectorLayer) {
    ...
    ClusterLayer.fromJSON = function fromJSON(json) {
        ...
        for (var i = 0; i < geoJSONs.length; i++) {
            var geo = maptalks__WEBPACK_IMPORTED_MODULE_0__["Geometry"].fromJSON(geoJSONs[i]);
            if (geo) {
                geometries.push(geo);
            }
        }
    ...
    ClusterLayer.prototype.addGeometry = function addGeometry(markers) {
        for (var i = 0, len = markers.length; i <= len; i++) {
            if (!markers[i] instanceof maptalks__WEBPACK_IMPORTED_MODULE_0__["Marker"]) {
    ...
    return ClusterLayer;
}(maptalks__WEBPACK_IMPORTED_MODULE_0__["VectorLayer"]);
...
ClusterLayer.registerRenderer('canvas', function (_maptalks$renderer$Ve) {
    ...
    _class.prototype.checkResources = function checkResources() {
        ...
        if (symbol !== this._symbolResourceChecked) {
            var res = maptalks__WEBPACK_IMPORTED_MODULE_0__["Util"].getExternalResources(symbol, true);
            if (res) {
    ...
    _class.prototype._getClustersToDraw = function _getClustersToDraw(zoomClusters) {
        this._markersToDraw = [];
        var map = this.getMap();
        var font = maptalks__WEBPACK_IMPORTED_MODULE_0__["StringUtil"].getFont(this._textSymbol),
            digitLen = maptalks__WEBPACK_IMPORTED_MODULE_0__["StringUtil"].stringLength('9', font).toPoint();
        ...
        for (var p in zoomClusters) {
            ...
            pExt = new maptalks__WEBPACK_IMPORTED_MODULE_0__["PointExtent"](pt.sub(width, height), pt.add(width, height));
            ...
            if (!zoomClusters[p]['textSize']) {
                ...
                zoomClusters[p]['textSize'] = new maptalks__WEBPACK_IMPORTED_MODULE_0__["Point"](digitLen.x * text.length, digitLen.y)._multi(1 / 2);
            }
    ...
    _class.prototype._refreshStyle = function _refreshStyle() {
        ...
        this._symbol = maptalks__WEBPACK_IMPORTED_MODULE_0__["MapboxUtil"].loadFunctionTypes(symbol, argFn);
        this._textSymbol = maptalks__WEBPACK_IMPORTED_MODULE_0__["MapboxUtil"].loadFunctionTypes(textSymbol, argFn);
    };
    ...
    _class.prototype._drawLayer = function _drawLayer(clusters) {
        ...
        if (layer.options['animation'] && this._animated && this._inout) {
            ...
            this._player = maptalks__WEBPACK_IMPORTED_MODULE_0__["animation"].Animation.animate({ 'd': dr }, { 'speed': layer.options['animationDuration'], 'easing': 'inAndOut' }, function (frame) {
    ...
    _class.prototype._drawCluster = function _drawCluster(pt, cluster, op) {
        ...
        if (this.layer.options['drawClusterText'] && cluster['textSize']) {
            maptalks__WEBPACK_IMPORTED_MODULE_0__["Canvas"].prepareCanvasFont(ctx, this._textSymbol);
            ctx.textBaseline = 'middle';
            var dx = this._textSymbol['textDx'] || 0;
            var dy = this._textSymbol['textDy'] || 0;
            var text = this._getClusterText(cluster);
            maptalks__WEBPACK_IMPORTED_MODULE_0__["Canvas"].fillText(ctx, text, pt.sub(cluster['textSize'].x, 0)._add(dx, dy));
        }
    ...
    _class.prototype._getSprite = function _getSprite() {
        ...
        var key = maptalks__WEBPACK_IMPORTED_MODULE_0__["Util"].getSymbolStamp(this._symbol);
        if (!this._spriteCache[key]) {
            this._spriteCache[key] = new maptalks__WEBPACK_IMPORTED_MODULE_0__["Marker"]([0, 0], { 'symbol': this._symbol })._getSprite(this.resources, this.getMap().CanvasClass);
        }
    ...
    _class.prototype._computeZoomGrid = function _computeZoomGrid(zoom) {
        ...
        for (var i = 0, len = points.length; i < len; i++) {
            ...
            if (!grids[key]) {
                grids[key] = {
                    'sum': new maptalks__WEBPACK_IMPORTED_MODULE_0__["Coordinate"](points[i].x, points[i].y),
                    'center': new maptalks__WEBPACK_IMPORTED_MODULE_0__["Coordinate"](points[i].x, points[i].y),
                    'count': 1,
                ...
                grids[key]['sum']._add(new maptalks__WEBPACK_IMPORTED_MODULE_0__["Coordinate"](points[i].x, points[i].y));
                grids[key]['count']++;
    ...
    return _class;
}(maptalks__WEBPACK_IMPORTED_MODULE_0__["renderer"].VectorLayerCanvasRenderer));
